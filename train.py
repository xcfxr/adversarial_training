...
def cutmix_data(X, y, alpha, mix=False):
    lam = np.random.beta(alpha, alpha)
    ...
    if mix:
        ...
        X.data[:, :, bbx1:bbx2, bby1:bby2] = X.data[rand_index, :, bbx1:bbx2, bby1:bby2] * (1 - lam2) + lam2 * \
                                             X.data[:, :, bbx1:bbx2, bby1:bby2]
        lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (X.size()[-1] * X.size()[-2]))
        lam = lam + (1 - lam) * lam2

    else:
        X.data[:, :, bbx1:bbx2, bby1:bby2] = X.data[rand_index, :, bbx1:bbx2, bby1:bby2]
        lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (X.size()[-1] * X.size()[-2]))
...
def main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_batches):
            ...
            if args.mixup:
                X, y_a, y_b, lam = cutmix_data(X, y, args.mixup_alpha)
