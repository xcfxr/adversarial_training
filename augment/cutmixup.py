...
class CutMixup(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for _ in range(self.m):
            ...
            lam2 = np.random.beta(self.beta, self.beta)
            rand_index = random.choice(range(len(self)))
            img2, lbl2 = self.dataset[rand_index]
            lbl2 = self.one_hot(lbl2)
            bbx1, bby1, bbx2, bby2 = rand_bbox(img.size(), lam)
            img[:, bbx1:bbx2, bby1:bby2] = lam2 * img2[:, bbx1:bbx2, bby1:bby2] + (1 - lam2) * img2[:, bbx1:bbx2, bby1:bby2]
            lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (img.size()[-1] * img.size()[-2]))
            lbl = lbl1 * lam + lbl2 * (1. - lam) * (1. - lam2) + lbl1 * (1-lam) * lam2
            m_imgs.append(img)
            m_lbls.append(lbl)
